...
const resultFile = "eth-neo-binding.csv";
...
async function getAllLrcHolderBalances() {
  ...
  for (const key of keys) {
    ...
    const balance = await lrcToken.methods.balanceOf(key).call();
    const balanceInt = Math.floor(balance/1e18);
    if (balanceInt === 0) {
      console.log("lrc balance is 0, skip.");
    } else {
...
function writeResultToFile() {
  const keys = [...bindingMap.keys()];
  keys.forEach((key) => {
    fs.appendFileSync(resultFile, key + "," + bindingMap.get(key) + "," +
                      lrcBalanceMap.get(key) + "\n");
  });
}
